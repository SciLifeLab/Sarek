...
are following the same annotation format to accomodate VCF files generated by Sarek to Scout.
...
RankScore. If there are multiple scores for a single genomic coordinate in the CSV, the very last one in the file will
be stored only.
